...
def transform(pose, target_v):
   # Calculates the transformations necessary to move the input pose from its original position to [0,0,1]
...
raw_pdbs = read_in_pdbs()  # imports poses - requires a "pdbs.list" file containing paths
prepped_pdbs = prep_poses(
   raw_pdbs)  # preps poses - makes all non-PRO (l&d) non-AIB residues into Alanines
df = pandas.read_csv(
   'crystals_from_point.csv')  # read FD & UN's materials info into a pandas DataFrame (df)

for pdb in prepped_pdbs:
   p_n = pdb.pdb_info().name().split('/')[-1]  # gets the pdb name for outputs later
   pdb_name = p_n[:-4]  # gets rid of the ".pdb" at the end of the pdb name
   ...
   df_sym = df1.append(
      df2)  # this df contains only applicable crystal space groups/cage types for a given scaffold
   ...
   for residue in range(1, int(total_res / sym) + 1):
      ...
      if pdb.residue_type(residue) == rts.name_map('ALA') or pdb.residue_type(
         ...
         LIG_poses = mut_to_ligand(
            pdb, residue
         )  # one residue at a time, mutate to a residue that is involved in one of the defined interactions. Stores the output in a dictionary w/ "a" = poses, "b" = interaction residue (his, asp, cys, etc.)

         for LIG_pose in LIG_poses:
            transform(LIG_pose, [1, 0, 0])
            rotamers = LIG_pose.residue(
               residue).get_rotamers()  # gets all of the backbone independent rotamers
            ...
            for rotamer in rotamers:
               ...
               ROT_pose = rosetta.protocols.grafting.return_region(LIG_pose, 1, LIG_pose.size())
               if _DEBUG:
               ...
               if dun_score < 3:  # scores the mutated residue's fa_dun to only get "good" rotamers (bb_dependent)
                  ...
                  if len(str_los) > 50:
                     output_data(pdb_name, residue, mut_res_name, pose_num, list_of_spacegroups)
